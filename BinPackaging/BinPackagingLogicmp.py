...
class BinAssignment:
    ...
    def generateResults(self,  orderData, availableBoxes, outputflName):
        ...
        dtOrders = orderData["ORDER_NO"].unique()
        
        numProcesses = cpu_count()
        with Pool(processes = numProcesses) as pool:
            for orders_subset in np.array_split(dtOrders, numProcesses):
                dt_subset = orderData[orderData["ORDER_NO"].isin(orders_subset)]
                futures.append(pool.apply_async(self.AssignBoxes, args = (dt_subset, availableBoxes,)))
